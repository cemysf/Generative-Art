...
import painter
from utils import QColor_HSV, printCallParameters, printElapsed, save, Perlin2D
...
def draw_flow_fields(*args, seed=random.randint(0, 100000000), colors = [200, 140, 70, 340, 280]):
    for i, color in enumerate(colors):
        print('Starting Image %s/%s' % (i + 1, len(colors)))
        draw_flow_field(*args, seed=seed, color=color)
...
def draw_flow_field(width, height, seed=random.randint(0, 100000000), color=200):
    ...
    for j in range(num):
        ...
        fname = f'image_{color}_{seed}'
        save(p, fname=fname, folder='.', overwrite=True)
        return p
